...
from home_assistant_intents import ErrorKey, get_languages
import yaml
...
from homeassistant.components.conversation.const import DEFAULT_EXPOSED_ATTRIBUTES
...
async def async_setup_entry(
    ...
    entity = DefaultAgentPlus(hass, config_entry.data["name"], {})
...
class DefaultAgentPlus(ConversationEntity):
    """Default agent for conversation agent."""

    # _attr_supported_features = ConversationEntityFeature.CONTROL

    def __init__(
        self, hass: core.HomeAssistant, name: str, config_intents: dict[str, Any]
    ) -> None:
        ...
        if isinstance(result, SentenceTriggerResult):
            ...
            if response_set_by_trigger:
                ...
                translations = await translation.async_get_translations(
                    self.hass, language, CONVERSATION_DOMAIN, [CONVERSATION_DOMAIN]
                )
                response_text = translations.get(
                    f"component.{CONVERSATION_DOMAIN}.agent.done", "Done"
                )
    ...
    def _load_intents(self, language: str) -> LanguageIntents | None:
        ...
        for x in ["base_" + language_variant, language_variant]:
            custom_sentences_dir = Path(self.hass.config.path("custom_sentences", x))
