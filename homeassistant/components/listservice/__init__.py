...
CALL_SCHEMA = vol.Schema({vol.Required('service'): cv.string,
    ...
    async def handle_call(call: ServiceCall):
        ...
        for k, v in dict(call.data.get("transforms", {})).items():
            ...
            if v == "list":
                ...
                result = [s for s in (s.strip() for s in str(to_transform).split(",")) if s]
            elif v == "intlist":
                result = [int(s) for s in (s.strip() for s in str(to_transform).split(",")) if s]
            elif v == "dict":
                result = dict(
                    (
                        (k.strip(), v.strip())
        ...
        _LOGGER.debug("Calling service %s.%s with data: %s (Response: %s)", domain, service, data, return_response);
        result = await hass.services.async_call(
            domain, service, service_data=data, blocking=True, context=call.context, target=call.data.get('target', None), return_response=return_response
        )

        if result is not None:
            return result

    hass.services.register(DOMAIN, "call", handle_call, CALL_SCHEMA, SupportsResponse.OPTIONAL)
